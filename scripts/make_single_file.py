...
function theoryDetail(id){const t=D.theories.find(x=>x.id===id);if(!t)return '<section class="wrap"><div class="empty">未找到该理论</div></section>';
 ...
 const tb=tbk?('<div style="margin-top:14px"><b>教科书级讲解</b>'+esc(tbk.intro||'')+(tbk.sections||[]).map(s=>'<div style="margin-top:10px"><b>'+esc(s.h||'')+'</b>'+(s.p||[]).map(x=>'<p style="color:#c3c9d6;font-size:14px">'+esc(x)+'</p>').join('')+(s.list||[]).map(x=>'<div style="color:#c3c9d6;font-size:14px">· '+esc(x)+'</div>').join('')+(s.table||[]).map(r=>'<div style="font-size:13px;color:#9aa2b1">'+esc(r.a)+'：'+esc(r.b)+'　→　'+esc(r.c)+'</div>').join('')+'</div>').join('')+(tbk.formulas||[]).map(f=>'<div class="quote">'+esc(f.tex||'')+'</div>').join('')+(tbk.model3d?'<div style="margin-top:12px;color:#9aa2b1;font-size:13px">3D 模型演示：'+esc(tbk.model3d.caption||'')+'（可交互的三维演示请打开离线完整版 site/ 或在线站点）</div>':'')+'</div>'):'';
