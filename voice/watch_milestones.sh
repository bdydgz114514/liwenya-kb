#!/bin/bash
# 里程碑守望：每当训练到达 25 的倍数存档，就提取小模型并出翻唱（BV1ENmhBTE5M）
source /root/miniconda3/etc/profile.d/conda.sh
conda activate rvc
cd /root/dsh/rvc/webui
export PYTHONPATH=/root/dsh/rvc/webui
LOG=/root/dsh/liwenya-kb/voice/train_rvc.log
WORK=/root/dsh/liwenya-kb/voice/cover
INDEX=logs/liwenya/added_IVF256_Flat_nprobe_1_liwenya_v2.index
mkdir -p "$WORK"
for m in 25 50 75 100 125 150; do
  while true; do
  ...
  echo "[milestone] epoch $m 存档完成，提取模型 $(date '+%T')"
  python extract_ckpt.py "$m" logs/liwenya/G_2333333.pth 2>&1 | tail -2
  if [ -f "assets/weights/liwenya_e${m}.pth" ]; then
    bash /root/dsh/liwenya-kb/voice/make_cover.sh "e${m}" "/root/dsh/rvc/webui/assets/weights/liwenya_e${m}.pth" "/root/dsh/rvc/webui/$INDEX" 2>&1 | tail -3
  fi
done
echo MILESTONES_DONE